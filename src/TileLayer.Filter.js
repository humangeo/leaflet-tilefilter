...
/**
 * An extensible image filter class
 */
...
/**
 * An image filter that uses a canvas to apply effects.  The render method can be overridden to make use of the various 
 * JavaScript canvas manipulation libraries out there (e.g. CamanJS)
 */
L.CanvasFilter = L.ImageFilter.extend({
	render: function () {
		...
		if (ctx) {
			...
			this._image.onload  = null; // to prevent an infinite loop
...
/**
 * Defines a basic filter that adjust the channels for a given canvas pixel
 */
L.CanvasChannelFilter = L.Class.extend({
	...
	/**
	 * Overrides this for simple filters
	 */
	...
	/**
	 * Override this for more complex filters
	 */
	render: function () {
...
/**
 * Contains a collection of canvas channel filters
 */
L.CanvasChannelFilters = {};

/**
 * Defines a grayscale filter
 */
...
/**
 * Loosely based on:  http://stackoverflow.com/questions/10521978/html5-canvas-image-contrast
 */
...
/**
 * Swaps the values in the specified channel positions
 */
...
/**
 * Adjust channel values using the specified matrix
 */
...
/**
 *  Combine canvas channel filters to produce a more complicated result
 */
...
/**
 * Adjusts the lightness/darkness of the image using the specified channel adjustments
 */
...
L.CanvasChannelFilters.HSLAdjust = L.CanvasChannelFilter.extend({
	options: {
		// Should be 0..360, 0..1, 0..1
		adjustments: [30, 0.0, 0.0]
	},
...
/**
 * Colors the tiles based on the specified channel
 */
L.CanvasChannelFilters.Colorize = L.CanvasChannelFilter.extend({
	...
	updateChannels: function (channels) {
		...
		channelIndices.splice(this.options.channel, 1);
      	...
      	channels[channelIndices[1]] = this.options.values[1]; // change value of other channels
...
/**
 * Applies an HTML5 CSS filter to the image
 */
...
/**
 * Combines a Canvas channel filter with a css filter, expanding the range of possibilities.  The
 * css filter(s) will be applied after the canvas channel filters
 */
L.CombinedFilter = L.ImageFilter.extend({

	// Set the channel filter that will be used - note that this can be a filter chain channel filter
	// which allows channel filters to be chained together
	setCanvasFilter: function (filter) {
		this.options.channelFilter = filter;
		return this.render();
	},
	
	// Set the CSS filter that will be used
...
/**
 * Convenience function for generating a CSS Filter from an array of filter strings
 */
...
/**
 * A collection of preset filters for quick use
 */
